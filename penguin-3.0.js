...
(function(window, undefined){
    ...
    function _DomNodeToString(n){
		...
		//REMOVED: Conversion from node to string using XML leaves xmlns attribute in Firefox
		//return new XMLSerializer().serializeToString(n);
		
    };
    
	/**
	 * Returns an array of elements from the document with a given attribute
	 * @param {String} attr
	 * @returns {Array}
	 */
    function _GetElementsByAttribute(attr){
    ...
    function _GetMarkedElements(el){
        ...
        var next = el; //Setting this to el.firstChild omits the current node check for marks
		var marks = {};
		var node;
        ...
        while((node = next)){
            ...
            if(node.nodeType === Node.ELEMENT_NODE && node.hasAttribute(_Config.templateMarkDataAttribute)){
                ...
                if(_Config.cleanTemplates){
                    node.removeAttribute(_Config.templateMarkDataAttribute);
                }
			...
			while(html){
				...
				if(!stack.last() || !special[stack.last()]){
					...
					if(html.indexOf("<!--") === 0){
						...
						//console.log("Found start tag @ " + html.indexOf("<"));
		...
		return function(html){
			...
			HTMLParser(html, {
				...
				start: function(tagName, attrs, unary) {
					...
					//console.log("Appending " + tagName + " to current parent node (" + (curParentNode !== frag ? curParentNode.tagName : "fragment") + ")");
					curParentNode.appendChild(elem);

					//Keep track of non-void elements
					if(!unary) {
						//console.log("Setting curParentNode to " + tagName);
						elems.push(elem);
						curParentNode = elem;
					}
					...
					if(!empty[tag]){
						//console.log("Found inline tag: " + tag);
						curParentNode = elems[elems.length - 1];
					}

					//console.log("Finished with " + tag);
					//console.log(curParentNode);

				},
				chars: function(text){

					//console.log("Appending text: \"" + text + "\" to parent (" + (curParentNode !== frag ? curParentNode.tagName : "fragment") + ")");
    ...
    */
    ...
    function _PrecompileTemplates(){
		...
		var elems = _GetElementsByAttribute(_Config.templateDataAttribute);
        var scripts = (function(){

            var o = {};
			var sourceArr = [];
            var scriptElems = Array.prototype.slice.call(document.scripts).filter(function(s){
				return s.hasAttribute(_Config.scriptFor) && s.type.toLowerCase() === _Config.scriptMimeType;
			});
			
			sourceArr.length = scriptElems.length;
			
            for(var i=0,l=scriptElems.length; i<l; ++i){
                
				//Remove any trailing semicolons (they're not valid when placed in an array)
				sourceArr[i] = scriptElems[i].textContent.replace(/(?!})\s*;\s*$/, "");
				
				//Set a property to come back to
				o[i] = scriptElems[i].getAttribute(_Config.scriptFor);
    ...
    function Template(o){
        ...
        if(_Config.cleanTemplates && !(this._node instanceof DocumentFragment)){
            this._node.removeAttribute(_Config.templateDataAttribute);
        }
        
        if(_Config.isBuffering){
            _TemplateBuffer[_TemplateBuffer.length] = this;
        }
    ...
    Object.defineProperties(Template.prototype, {
		 ...
		 * @param {Array} coll Array of items to be bound to a template instance
		 */
		DataBindCollection: {
			value: function(coll){
			...
			value: function(a, b, c){
				...
				for(var i=0; i<mods.length; ++i){
					...
					if(mods[i].Property === null){
						...
						else if(modType === "string" || modType === "boolean" || modType === "number"){
	...
	Object.defineProperties(TemplateCollection.prototype, {
		...
		/**
		 * Generates a document fragment with each of the template elements within it
		 * @returns {DocumentFragment}
		 */
		Node: {
			get: function(){
